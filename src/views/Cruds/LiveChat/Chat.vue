    ...
    <div class="chat-messages" ref="messagesContainer">
      <div v-for="(group, index) in groupedMessages" :key="index">
        ...
        <div
          ...
          :class="[
            isMessageFromAdmin(message) ? 'received' : 'sent',
...
export default {
  ...
  methods: {
    isMessageFromAdmin(message) {
      if (!message || message.from_id === null || message.from_id === undefined) {
        return false;
      }
      
      const adminId = this.getAuthenticatedUserData?.id;
      if (!adminId || adminId === null || adminId === undefined) {
        return false;
      }
      
      // Convert both to strings first, then to numbers for reliable comparison
      const messageFromId = parseInt(String(message.from_id), 10);
      const adminIdNum = parseInt(String(adminId), 10);
      
      // Check if conversion was successful (not NaN)
      if (isNaN(messageFromId) || isNaN(adminIdNum)) {
        return false;
      }
      
      return messageFromId === adminIdNum;
    },
    async fetchChat() {
      try {
